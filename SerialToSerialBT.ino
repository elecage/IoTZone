//This example code is in the Public Domain (or CC0 licensed, at your option.)
//By Evandro Copercini - 2018
//
//This example creates a bridge between Serial and Classical Bluetooth (SPP)
//and also demonstrate that SerialBT have the same functionalities of a normal Serial

#include "BluetoothSerial.h"

//#define USE_PIN // Uncomment this to use PIN during pairing. The pin is specified on the line below
const char *pin = "1234"; // Change this to more secure PIN.

String device_name = "ESP32-BT-Slave1";

#if !defined(CONFIG_BT_ENABLED) || !defined(CONFIG_BLUEDROID_ENABLED)
#error Bluetooth is not enabled! Please run `make menuconfig` to and enable it
#endif

#if !defined(CONFIG_BT_SPP_ENABLED)
#error Serial Bluetooth not available or not enabled. It is only available for the ESP32 chip.
#endif

BluetoothSerial SerialBT;

const int freq = 5000;
const int resolution = 7;

const int motorAPin1 = 16;
const int motorAPin2 = 17;
const int motorBPin1 = 18;
const int motorBPin2 = 19;

const int motorAChannel1 = 0;
const int motorAChannel2 = 1;
const int motorBChannel1 = 2;
const int motorBChannel2 = 3;


byte recvData[6];
int dIndex = 0;
void setup() {
  ledcSetup(motorAChannel1, freq, resolution);
  ledcSetup(motorAChannel2, freq, resolution);
  ledcSetup(motorBChannel1, freq, resolution);
  ledcSetup(motorBChannel2, freq, resolution);
  
  ledcAttachPin(motorAPin1, motorAChannel1);
  ledcAttachPin(motorAPin2, motorAChannel2);
  ledcAttachPin(motorBPin1, motorBChannel1);
  ledcAttachPin(motorBPin2, motorBChannel2);
  
  Serial.begin(115200);
  SerialBT.begin(device_name); //Bluetooth device name
  Serial.printf("The device with name \"%s\" is started.\nNow you can pair it with Bluetooth!\n", device_name.c_str());
  //Serial.printf("The device with name \"%s\" and MAC address %s is started.\nNow you can pair it with Bluetooth!\n", device_name.c_str(), SerialBT.getMacString()); // Use this after the MAC method is implemented
  #ifdef USE_PIN
    SerialBT.setPin(pin);
    Serial.println("Using PIN");
  #endif
}

byte currData, prevData;

void setMotor(int left, int right)
{
      ledcWrite(motorAChannel1, left);
    ledcWrite(motorAChannel2, 0);
    ledcWrite(motorBChannel1, right );
    ledcWrite(motorBChannel2, 0);
  /*
  int l, r;
  l = left - 128;
  r = right - 128;
  if(l > 15)
  {
    ledcWrite(motorAChannel1, l*2 );
    ledcWrite(motorAChannel2, 0);
    Serial.print('>');
  }
  else if(l < -15)
  {
    ledcWrite(motorAChannel1, 0);
    ledcWrite(motorAChannel2, -l*2 );
    Serial.print('<');
  }
  else
  {
    ledcWrite(motorAChannel1, 0);
    ledcWrite(motorAChannel2, 0 );
  }
  if(r > 15)
  {
    ledcWrite(motorBChannel1, r*2 );
    ledcWrite(motorBChannel2, 0);
    Serial.print('^');
  }
  else if(r < -15)
  {
    ledcWrite(motorBChannel1, 0);
    ledcWrite(motorBChannel2, -r*2 );
    Serial.print('v');
  }
  else
  {
    ledcWrite(motorBChannel1, 0);
    ledcWrite(motorBChannel2, 0);
  }
        Serial.print(l);
        Serial.print(" : ");
        Serial.println(r);
        */
}
void loop() {
  int x, y;
  static int a = 0;
 
  while(SerialBT.available()) {
     currData = SerialBT.read();
     recvData[dIndex] = currData;
     if(currData == 0xfa && prevData == 0xfd)
     {
        
        setMotor((int)recvData[3], (int)recvData[3]);
        dIndex = 0;
     }
     else
     {
      dIndex++;
     }
     prevData = currData;
  }
  
  delay(50);
}
